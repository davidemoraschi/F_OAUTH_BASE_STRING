/* Formatted on 2010/08/17 20:05 (Formatter Plus v4.8.8) */
CREATE OR REPLACE PACKAGE BODY almagesto.utl_linkedin
AS
   ...
   AS
      ...
      v_user_params                 VARCHAR2 (1024)
                        := '<?xml version="1.0" encoding="UTF-8"?><current-status>' || (p_status)
                           || '</current-status>';
   ...
   BEGIN
      ...
      var_http_request := UTL_HTTP.begin_request (url => 'http://www.quotationspage.com/data/mqotd.rss', method => 'GET');
