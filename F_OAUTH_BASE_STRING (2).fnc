/* Formatted on 2010/08/10 15:54 (Formatter Plus v4.8.8) */
CREATE OR REPLACE FUNCTION almagesto.f_oauth_base_string(p_nonce in VARCHAR2)
   RETURN VARCHAR2
AS
...
BEGIN
   ...
   SELECT    'GET'
          ...
          || urlencode (   'oauth_callback'
                        ...
                        || 'oauth_consumer_key'
                        || '='
                        || urlencode ('NHx8CZzuf9XEF6C2ksimwDkS7Fw=')
                        || '&'
                        || 'oauth_nonce'
                        || '='
                        || p_nonce
